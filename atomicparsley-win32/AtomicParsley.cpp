...
void APar_FreeMemory() {
	for(int iter=0; iter < atom_number; iter++) {
		if (parsedAtoms[iter].AtomicData != NULL) {
			//free(parsedAtoms[iter].AtomicData);	   ///BUG/// Posible bug, comment me.
		}
	}
	return;
}
...
void APar_AtomicWriteTest(short AtomicNumber, bool binary) {
	...
	memset(indy_atom_path, '\0', sizeof(char)*MAXPATHLEN);
...
void APar_ExtractAAC_Artwork(short this_atom_num, char* pic_output_path, short artwork_count) {
	...
	memset(base_outpath, '\0', sizeof(char)*MAXPATHLEN);
	...
	memset(art_payload, '\0', sizeof(char)*(parsedAtoms[this_atom_num].AtomicLength-16));
...
void APar_ExtractDataAtom(int this_atom_number) {
	if ( source_file != NULL ) {
		...
		memset(parent_atom_name, '\0', sizeof(char)*4);
		...
		if (thisAtom.AtomicLength > min_atom_datasize ) {
			...
			if (thisAtom.AtomicDataClass == AtomicDataClass_Text) {
				...
				memset(primary_number_data, '\0', sizeof(char)*4);
				...
				//free(primary_number_data);		///BUG/// Comment out if crash.
				primary_number_data=NULL;
				//free(data_payload);
				data_payload = NULL;
			}									///BUG/// Comment out if crash.
		}
		//free(parent_atom_name);					///BUG/// Comment out if crash.
	}
	return;
}
...
void APar_PrintDataAtoms(const char *path, bool extract_pix, char* pic_output_path) {
	...
	if (artwork_count != 0) {
		if (artwork_count == 1) {
			fprintf(stdout, "Atom \"covr\" contains: %d piece of artwork\n", artwork_count);
		} else {
			fprintf(stdout, "Atom \"covr\" contains: %d pieces of artwork\n", artwork_count);
		}
	}
	return;
}
...
bool APar_TestforChildAtom(char *fileData, uint32_t sizeofParentAtom, char* atom) {
	...
	//If we crash, don't use dynamic memory?  ///HACK///
...
short APar_DetermineDataType(char* atom, bool uuid_type) {
	...
	memset(data_type, '\0', sizeof(char)*4);
	int data_offset = 8;
	if (uuid_type) {
		data_offset = 12;
	}
...
void APar_AddMetadataArtwork(const char* m4aFile, const char* artworkPath, char* env_PicOptions) {
	...
	if (_strnicmp(artworkPath, "REMOVE_ALL", 10) == 0) {
...
void APar_WriteFile(const char* m4aFile, bool rewrite_original) {
	...
	memset(temp_file_name, '\0', strlen(m4aFile)+12);
	...
	if (rewrite_original) {
		...
		originating_file = fopen(m4aFile, "wb");
		temp_file = NULL;
		temp_file = fopen(temp_file_name, "rb"); //reopens as read-only
